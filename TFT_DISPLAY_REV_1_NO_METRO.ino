/***************************************************
  ...
  ******************************************************************************
  * @file    TFT_DISPLAY_REV_1_NO_METRO.ino
  ...
  *
  * This project "VTFT_DISPLAY_REV_1_NO_METRO", define an experimental open platform to build
...
int bottoneRxTx = A1;
int bottoneBfo = 8;
int analogIn = A7;/////////ingresso ANALOGICO per S-METER
...
if (digitalRead(bottoneRxTx) == LOW){  
    var5= 1; //memorizza sulla variabile il valore/bottone 
    if (var5 != var6){   /////////riconosce se viene premuto O rilasciato il tasto PTT 
    trasmetti(); 
    var6 = var5;  
  }
}
...
void Band(){  
   ...
   if (band == 1) {
      freq = 1860000;
      tft.fillRoundRect(1, 10, 99, 70,5,ILI9341_RED); //BANDS CORNICE
      tft.drawRoundRect(1, 10, 99, 70,5,ILI9341_WHITE); //BANDS CORNICE
      tft.setCursor(24,22); 
...
///////////////////////////R-TX PLL CHANGE VALUE & SI5351 CLK OUTPUT----HAM RADIO USE ONLY---
void trasmetti(){
...
void grafica(){
  ///smeter---------SCALA GRAFICA SOLO PER ESEMPIO!!!
      tft.setTextSize(1);
      tft.setTextColor(WHITE);
